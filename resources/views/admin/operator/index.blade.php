                    ...
                    @foreach ($data as $item)
                        <tr
                            ...
                            <td class="px-6 py-4 flex flex-wrap">
                                <form action="{{ route('operator.destroy', $item->id) }}" method="post" id="form-delete-{{ $item->id }}"">
                                    @csrf
                                    @method("DELETE")
                                    <button type="button" 
                                        data-id="{{ $item->id }}"
                                        class="text-white btn-delete bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium  text-sm px-5 py-2.5 text-center rounded-lg me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">Hapus</button>
